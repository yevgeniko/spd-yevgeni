...
def load_classifier_and_predict(saturation, temp, pulse):
    
    clf = joblib.load('health_classifier.pkl')
    prediction = int(clf.predict([[saturation, temp, pulse]]))
    return prediction
...
if __name__ == "__main__":
    ...
    pulse = float(sys.argv[3])

    new_patient_features = [[[saturation, temp, pulse]]]

    predicted_health_status = load_classifier_and_predict(saturation, temp, pulse)
